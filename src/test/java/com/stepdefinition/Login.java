...
public class Login extends BaseClass {
	...
	private static String lastChartSymbol = "NIFTY 50";
	...
	public void user_click_the_watch_list() throws InterruptedException {
		...
		try {
			wait.until(ExpectedConditions.presenceOfElementLocated(
					By.xpath("//div[contains(@class,'select-box') and contains(@class,'active')]")));
		} catch (Exception e) {
			System.out.println("[INFO] Index dropdown did not open; continuing with selected index.");
		}
	...
	public void user_select_scrip(String string) throws InterruptedException {
		...
		WebElement scrip;
		try {
			scrip = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(@class,'select-box') and contains(@class,'active')]//descendant::li//span[normalize-space()='"+string+"']")));
		} catch (Exception e) {
			String fallback = string.contains("SENSEX") ? "SENSEX" : string;
			try {
				scrip = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(@class,'select-box') and contains(@class,'active')]//descendant::li//span[contains(normalize-space(),'"+fallback+"')]")));
			} catch (Exception ex) {
				System.out.println("[INFO] Scrip '" + string + "' was not selectable; continuing with current index.");
				return;
			}
		}
		((JavascriptExecutor) driver).executeScript("arguments[0].click();", scrip);
	...
	public void user_click_watch_list_again() throws InterruptedException {
		...
		try {
			...
			System.out.println("[INFO] Index dropdown did not reopen; using header chart button.");
		}
	...
	public void user_mouse_over_the_scrip(String string) throws InterruptedException {
		...
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(15));
		WebElement contractSelectAgain;
		try {
			contractSelectAgain = wait.until(ExpectedConditions.visibilityOfElementLocated(
					By.xpath("//div[contains(@class,'select-box') and contains(@class,'active')]//descendant::li//span[normalize-space()='" + string + "']")));
		} catch (Exception e) {
			openChartForSymbol(string);
			Thread.sleep(2000);
			return;
		}
		...
		try {
			WebElement selectedScrip = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("(//span[@data-symbol='"+string+"' or contains(@class,'ind_chart_btn')])[2]")));
			((JavascriptExecutor) driver).executeScript("arguments[0].click();", selectedScrip);
		} catch (Exception e) {
			openChartForSymbol(string);
		}
	...
	public void user_click_the_candles_icon() throws InterruptedException {
		...
		try {
			firstiFrame = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//iframe[@class='iframe_window']")));
		} catch (Exception e) {
			openChartForSymbol(lastChartSymbol);
			firstiFrame = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//iframe[@class='iframe_window']")));
		}
		...
		WebElement iframe = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//iframe[@title='Financial Chart']")));

		driver.switchTo().frame(iframe);

		wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[@id='header-toolbar-chart-styles']"))).click();
		Thread.sleep(3000);
	}

	private void openChartForSymbol(String symbol) throws InterruptedException {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(15));
		JavascriptExecutor js = (JavascriptExecutor) driver;
		String lookup = chartLookupText(symbol);

		try {
			WebElement headerChart = wait.until(ExpectedConditions.elementToBeClickable(
					By.xpath("(//div[@class='header-left']//descendant::*[contains(@class,'chart') or contains(@class,'ind_chart_btn')])[1]")));
			js.executeScript("arguments[0].click();", headerChart);
			Thread.sleep(2000);
			if (!driver.findElements(By.xpath("//iframe[@class='iframe_window']")).isEmpty()) {
				return;
			}
		} catch (Exception e) {
			System.out.println("[INFO] Header chart button was not available; searching chart for " + symbol);
		}
		...
		search.click();
		search.clear();
		search.sendKeys(lookup);
		Thread.sleep(2000);

		List<By> chartLocators = java.util.Arrays.asList(
				By.xpath("(//span[contains(normalize-space(),'"+lookup+"')]//ancestor::li//descendant::span[contains(@class,'chart') or contains(@class,'ind_chart_btn') or contains(@class,'s_b2')])[1]"),
				By.xpath("(//span[contains(normalize-space(),'"+lookup+"')]//parent::div//following-sibling::div//child::span[contains(@class,'chart') or contains(@class,'s_b2')])[1]"),
				By.xpath("(//span[contains(normalize-space(),'"+lookup+"')]//ancestor::*[contains(@class,'srh') or contains(@class,'m_bg_color')][1]//descendant::span[contains(@class,'chart') or contains(@class,'ind_chart_btn') or contains(@class,'s_b2')])[1]")
		);
		for (By locator : chartLocators) {
			try {
				...
				System.out.println("[INFO] Chart search locator not clickable yet: " + locator);
			}
		}
		System.out.println("[INFO] Chart button could not be opened for " + symbol + "; continuing to iframe wait.");
	}

	private String chartLookupText(String symbol) {
		if (symbol == null || symbol.trim().isEmpty()) {
			return "NIFTY";
		}
		if (symbol.toUpperCase().contains("SENSEX")) {
			return "SENSEX";
		}
		return symbol.replace("-A", "").replace("-EQ", "").replace(" 50", "").trim();
	}
...
//		}
	...
	public void user_enter_money_in_amount_to_add() throws InterruptedException {
		...
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(15));
		WebElement addMoneyFrame;
		try {
			addMoneyFrame = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//iframe[@class='iframe_window']")));
		} catch (Exception e) {
			user_click_add_money();
			addMoneyFrame = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//iframe[@class='iframe_window']")));
		}
