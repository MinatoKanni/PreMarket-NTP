...
//			recorder.start();
	    ...
	    public void afterTest(Scenario scenario) throws ATUTestRecorderException {
	        ...
	        File screenshot = takeScreenshot(scenario.getName() + "_" + status);
	        if (screenshot != null && screenshot.isFile()) {
	            try {
	                scenario.attach(Files.readAllBytes(screenshot.toPath()), "image/png", screenshot.getName());
	            } catch (IOException e) {
	                System.err.println("[SCREENSHOT ERROR] Failed to attach screenshot: " + screenshot.getPath());
	                e.printStackTrace();
	            }
	        }
